...
def get_eval_state(
  ...
  eval_negatives_ids = torch.as_tensor(all_item_ids).to(device).unsqueeze(
      0)  # [1, X]
  ...
  return EvalState(all_item_ids=set(all_item_ids),
                   candidate_index=candidates,
                   top_k_module=top_k_module_fn(eval_negative_embeddings,
                                                eval_negatives_ids))
...
def eval_metrics_v2_from_tensors(
  ...
  num_batches = ((shared_input_embeddings.size(0) + user_max_batch_size - 1) //
                 user_max_batch_size)
  eval_top_k_ids_all = []
  eval_top_k_prs_all = []
  for mb in range(num_batches):
    eval_top_k_ids, eval_top_k_prs, _ = eval_state.candidate_index.get_top_k_outputs(
        query_embeddings=shared_input_embeddings[mb *
                                                 user_max_batch_size:(mb + 1) *
                                                 user_max_batch_size, ...],
        top_k_module=eval_state.top_k_module,
        k=k,
        invalid_ids=seq_features.past_ids[mb * user_max_batch_size:(mb + 1) *
                                          user_max_batch_size, :]
  ...
  eval_ranks = torch.where(eval_rank_indices == k, MAX_K + 1,
                           eval_rank_indices + 1)
...
def _avg(x: torch.Tensor, world_size: int) -> float:
  _sum_and_numel = torch.tensor([x.sum(), x.numel()],
                                dtype=torch.float32,
                                device=x.device)
  if world_size > 1:
