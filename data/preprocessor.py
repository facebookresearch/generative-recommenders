...
class MovielensDataProcessor(DataProcessor):
  ...
  def __init__(self,
               ...
               expected_max_item_id: Optional[int] = None) -> None:
  ...
  def preprocess_rating(self) -> int:
    ...
    if self._convert_timestamp:
      ratings['unix_timestamp'] = pd.to_datetime(ratings['unix_timestamp'],
                                                 unit='s')
    ...
    seq_ratings_data_train = seq_ratings_data[seq_ratings_data['user_id'] <=
                                              user_id_split]
    ...
    seq_ratings_data_test = seq_ratings_data[seq_ratings_data['user_id'] >
                                             user_id_split]
    ...
    if self.expected_num_unique_items() is not None:
      assert self.expected_num_unique_items(
      ) == num_unique_items, f"Expected items: {self.expected_num_unique_items()}, got: {num_unique_items}"
...
class AmazonDataProcessor(DataProcessor):
  ...
  def preprocess_rating(self) -> int:
    ...
    item_id_count = ratings['item_id'].value_counts().rename_axis(
        'unique_values').reset_index(name='item_count')
    user_id_count = ratings['user_id'].value_counts().rename_axis(
        'unique_values').reset_index(name='user_count')
    ratings = ratings.join(item_id_count.set_index("unique_values"),
                           on="item_id")
    ratings = ratings.join(user_id_count.set_index("unique_values"),
                           on="user_id")
    ...
    seq_ratings_data = seq_ratings_data[seq_ratings_data["item_ids"].apply(len)
                                        >= 5]
    ...
    if self.expected_num_unique_items() is not None:
      assert self.expected_num_unique_items(
      ) == num_unique_items, f"expected: {self.expected_num_unique_items()}, actual: {num_unique_items}"
