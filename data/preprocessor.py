...
class MovielensDataProcessor(DataProcessor):
  ...
  def preprocess_rating(self) -> int:
    ...
    if self._prefix == "ml-1m":
        ...
        movies = pd.read_csv(
            f"tmp/{self._prefix}/movies.dat", sep="::", names=["movie_id", "title", "genres"], encoding="iso-8859-1",
        )
        ...
        ratings.rename(
            columns={'userId': 'user_id', 'movieId': 'movie_id', 'timestamp': 'unix_timestamp'},
            inplace=True,
        )
        ...
        movies = pd.read_csv(
            f"tmp/{self._prefix}/movies.csv", sep=",", encoding="iso-8859-1",
        )
    ...
    print(f"{self._prefix} #item before normalize: {len(set(ratings['movie_id'].values))}")
    print(f"{self._prefix} max item id before normalize: {max(set(ratings['movie_id'].values))}")
    ...
    seq_ratings_data_train = seq_ratings_data[seq_ratings_data['user_id'] <= user_id_split] 
    seq_ratings_data_train.sample(frac=1).reset_index().to_csv(
        self.sasrec_format_csv_by_user_train(), index=False, sep=",",
    )
    seq_ratings_data_test = seq_ratings_data[seq_ratings_data['user_id'] > user_id_split]
    seq_ratings_data_test.sample(frac=1).reset_index().to_csv(
        self.sasrec_format_csv_by_user_test(), index=False, sep=","
    )
    print(f"{self._prefix}: train num user: {len(set(seq_ratings_data_train['user_id'].values))}")
    print(f"{self._prefix}: test num user: {len(set(seq_ratings_data_test['user_id'].values))}")

    #print(seq_ratings_data)
    if self.expected_num_unique_items() is not None:
        assert self.expected_num_unique_items() == num_unique_items, f"Expected items: {self.expected_num_unique_items()}, got: {num_unique_items}"
...
class AmazonDataProcessor(DataProcessor):
    def __init__(
        ...
        super().__init__(prefix, expected_num_unique_items=expected_num_unique_items, expected_max_item_id=None)
        self._download_path = download_path
        self._saved_name = saved_name
        self._prefix = prefix
    ...
    def preprocess_rating(self) -> int:
        ...
        print(f"{self._prefix} #user before filter: {len(set(ratings['user_id'].values))}")
        print(f"{self._prefix} #item before filter: {len(set(ratings['item_id'].values))}")
        ...
        print(f"{self._prefix} #user after filter: {len(set(ratings['user_id'].values))}")
        ...
        seq_ratings_data.sample(frac=1).reset_index().to_csv(self.output_format_csv(), index=False, sep=",")

        if self.expected_num_unique_items() is not None:
            assert self.expected_num_unique_items() == num_unique_items, f"expected: {self.expected_num_unique_items()}, actual: {num_unique_items}"
