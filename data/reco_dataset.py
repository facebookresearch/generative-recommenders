...
def get_reco_dataset(
  ...
  if dataset_name == "ml-1m" or dataset_name == "ml-20m":
    ...
    item_features: ItemFeatures = ItemFeatures(
        ...
        values=[
            torch.zeros((dp.expected_max_item_id() + 1, max_jagged_dimension),
                        dtype=torch.int64),
            torch.zeros((dp.expected_max_item_id() + 1, max_jagged_dimension),
                        dtype=torch.int64),
            torch.zeros((dp.expected_max_item_id() + 1, max_jagged_dimension),
    ...
    for df_index, row in items.iterrows():
      ...
      item_features.lengths[0][movie_id] = min(len(genres_vector),
                                               max_jagged_dimension)
      item_features.lengths[1][movie_id] = min(len(titles_vector),
                                               max_jagged_dimension)
      item_features.lengths[2][movie_id] = min(len(years_vector),
                                               max_jagged_dimension)
      for f, f_values in enumerate([genres_vector, titles_vector,
                                    years_vector]):
