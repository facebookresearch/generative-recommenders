...
class FlashAttnBwdPostprocessConvertdQ {
 public:
  ...
  static_assert(
      !IsSm90 || kNThreads % cutlass::NumThreadsPerWarpGroup == 0,
  ...
  using SmemLayoutdQaccum = std::conditional_t<
      IsSm90,
      Layout<Shape<
          Int<kBlockM * kHeadDim / NumdQWarpGgroups>,
          Int<NumdQWarpGgroups>>>,
  ...
  void operator()(Params const& params, char* smem_buf) {
    ...
    Tensor sdQaccum_flat = make_tensor(
        make_smem_ptr(shared_storage.smem_dqacc.data()),
        SmemLayoutdQaccumFlat{});
    ...
    if constexpr (IsSm90) { // Use BulkCopy
      ...
      if (thread_idx == 0) {
        ...
        copy(
            bulk_copy.with(
                *reinterpret_cast<uint64_t*>(&shared_storage.barrier_dQaccum)),
            gdQaccum,
            sdQaccum_flat);
      }
      __syncthreads();
      ...
      auto g2s_thr_copy_dQaccum =
          g2s_tiled_copy_dQaccum.get_thread_slice(thread_idx);
...
#pragma unroll
    ...
    Tensor mdQ = make_tensor(
        make_gmem_ptr(params.ptr_dQ), params.shape_dQ, params.stride_dQ)(
        _, _, bidh, !Jagged ? bidb : 0);
    Tensor gdQ = local_tile(
        domain_offset(make_coord(seqlen_info.offset, _0{}), mdQ),
        TileShape_MK{},
    ...
    Tensor tdQcdQ = gmem_thr_copy_dQ.partition_D(
        cute::make_identity_tensor(TileShape_MK{}));
...
#pragma unroll
    ...
    flash::copy<
        ...
        tdQcdQ,
        tdQpdQ,
        std::min(seqlen_info.seqlen - m_block * kBlockM, kBlockM));
  }
};
