...
def hstu_preprocess_and_attention(
    ...
    if kernel == HammerKernel.TRITON:
        u, attn_output = triton_hstu_preprocess_and_attention(
            x=x,
            norm_weight=norm_weight.to(x.dtype),
            norm_bias=norm_bias.to(x.dtype),
            norm_eps=norm_eps,
            num_heads=num_heads,
            attn_dim=attn_dim,
            hidden_dim=hidden_dim,
            uvqk_weight=uvqk_weight.to(x.dtype),
            uvqk_bias=uvqk_bias.to(x.dtype),
