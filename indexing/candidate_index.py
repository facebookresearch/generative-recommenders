...
class CandidateIndex(object):
  ...
  def filter_invalid_ids(
    ...
    if self._ids.size(0) == 1:
      # ((1, X, 1) == (B, 1, N)) -> (B, X)
      invalid_mask, _ = (
          self._ids.unsqueeze(2) == invalid_ids.unsqueeze(1)).max(dim=2)
      ...
      jagged_embeddings = self._embeddings.expand(B, -1,
                                                  -1).reshape(-1,
                                                              D)[valid_1d_mask]
  ...
  def get_top_k_outputs(
    ...
    if invalid_ids is not None:
      id_is_valid = ~((top_k_prime_ids.unsqueeze(2)
                       == invalid_ids.unsqueeze(1)).max(2)[0])  # [B, K + N_0]
      id_is_valid = torch.logical_and(
          id_is_valid,
          torch.cumsum(id_is_valid.int(), dim=1) <= k)
      # [[1, 0, 1, 0], [0, 1, 1, 1]], k=2 -> [[0, 2], [1, 2]]
      top_k_rowwise_offsets = torch.nonzero(id_is_valid,
                                            as_tuple=True)[1].view(-1, k)
      top_k_scores = torch.gather(top_k_prime_scores,
                                  dim=1,
    ...
    if return_embeddings:
      ...
      expanded_embeddings = self.embeddings.repeat_interleave(
          r, dim=0) if r > 1 else self.embeddings
      top_k_embeddings = batch_gather_embeddings(rowwise_indices=top_k_indices,
