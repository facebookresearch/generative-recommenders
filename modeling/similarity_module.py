...
class InteractionModule(torch.nn.Module):
  ...
  def interaction(
      ...
      target_embeddings: Optional[
          torch.Tensor] = None,  # [1, X, D'] or [B, X, D']
  ) -> torch.Tensor:
    pass
...
class GeneralizedInteractionModule(InteractionModule):
  ...
  def interaction(
    ...
    torch._assert(
        len(input_embeddings.size()) == 2,
        "len(input_embeddings.size()) must be 2")
    torch._assert(
        len(target_ids.size()) == 2, "len(target_ids.size()) must be 2")
    ...
    with torch.autocast(enabled=True, dtype=torch.bfloat16, device_type="cuda"):
      return self._ndp_module(
          ...
          item_sideinfo=self.get_item_sideinfo(
              item_ids=target_ids),  # [1/B, X, self._item_sideinfo_dim]
          item_ids=target_ids,
