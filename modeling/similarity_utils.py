...
def create_mol_interaction_module(
  ...
  mol_module = MoLSimilarity(
      ...
      (torch.nn.
       ...
       if item_hidden_dim > 0 else torch.nn.Sequential(
           torch.nn.Dropout(p=item_dropout_rate),
           torch.nn.Linear(
               in_features=input_dim,
               out_features=output_dim,
      ...
      gating_context_only_partial_fn=lambda input_dim, output_dim: torch.nn.
      Sequential(
          torch.nn.Linear(
              in_features=input_dim,
      ...
      gating_item_only_partial_fn=lambda input_dim, output_dim: torch.nn.
      Sequential(
      ...
      if gating_qi_hidden_dim > 0 else torch.nn.Sequential(
          torch.nn.Dropout(p=gating_qi_dropout_rate),
          torch.nn.Linear(
              in_features=input_dim,
              out_features=output_dim,
      ...
      gating_normalization_fn=lambda _: SoftmaxDropoutCombiner(
          dropout_rate=gating_softmax_dropout_rate, eps=1e-6),
      eps=eps,
  ...
  interaction_module_debug_str = (
      ...
      (f"-q{query_hidden_dim}d{query_dropout_rate}geglu"
       if query_hidden_dim > 0 else f"-qd{query_dropout_rate}") +
      ("-i_id" if item_use_identity_fn else
       (f"-{item_hidden_dim}d{item_dropout_rate}-geglu"
        if item_hidden_dim > 0 else f"-id{item_dropout_rate}")) +
      (f"-gq{gating_query_hidden_dim}" if gating_query_fn else "") +
      (f"-gi{gating_item_hidden_dim}d{gating_item_dropout_rate}"
       if gating_item_fn else "") +
